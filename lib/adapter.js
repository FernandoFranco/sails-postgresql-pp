...
module.exports = (function () {
  ...
  var adapter = {
    ...
    registerConnection: function (connection, collections, cb) {
      ...
      // Add Postgre Functions
      _addFunctions(connection.identity, function (err, result) {
        if (err) {
          return cb(err);
        }
    ...
    create: function (connectionName, table, data, cb) {
      spawnConnection(connectionName, function __CREATE__(client, cb) {
        ...
        getCustomSchema(options, tableName);
      ...
      spawnConnection(connectionName, function __FIND__(client, done) {
        ...
        Cursor({
          ...
          $populateBuffers: function populateBuffers(options, next) {
            ...
            getCustomSchema(_options, tableName);
    ...
    metas: function (connectionName, table, options, cb) {
      adapter._schemaName = (options && options.schemaName) || sqlOptions.schemaName[table] || 'public';
      return this;
    },

    // Clone Table intro another Schema
    cloneToSchema: function (connectionName, table, options, cb) {
      if (!options) {
        throw new Error('Options not found');
      }

      options.oldSchema = options.oldSchema || sqlOptions.schemaName[table];

      if (!options.oldSchema) {
        throw new Error('oldSchema not found');
      }

      if (!options.newSchema) {
        throw new Error('newSchema not found');
      }

      adapter.createSchema(connectionName, table, options.newSchema, function (err, result) {
        if (err) {
          return cb(err);
        }

        table = table ? '"' + table + '"' : 'NULL';
        var _query = 'SELECT clone_schema("' + options.oldSchema + '","' + options.newSchema + '",' + table + ');';
        adapter.query(connectionName, table, _query, null, cb);
      });
    }
  };
  ...
  function getCustomSchema(options, tableName) {
    if (adapter._schemaName) {
      options.schemaName[tableName] = adapter._schemaName;
    }
    delete adapter._schemaName;
  }

  function _addFunctions(connectionName, cb) {
    var _cloneSchema = "CREATE OR REPLACE FUNCTION sails_clone_schema(source_schema text, dest_schema text, table_ text) RETURNS void AS $BODY$ DECLARE object text; buffer text; default_ text; column_ text; BEGIN EXECUTE 'CREATE SCHEMA ' || dest_schema ;FOR object IN SELECT sequence_name::text FROM information_schema.SEQUENCES WHERE sequence_schema = source_schema LOOP EXECUTE 'CREATE SEQUENCE ' || dest_schema || '.' || object; END LOOP;FOR object IN SELECT TABLE_NAME::text FROM information_schema.TABLES WHERE table_schema = source_schema AND (TABLE_NAME::text = table_ OR table_ = '' OR table_ IS NULL) LOOP buffer := dest_schema || '.' || object; EXECUTE 'CREATE TABLE ' || buffer || ' (LIKE ' || source_schema || '.' || object || ' INCLUDING CONSTRAINTS INCLUDING INDEXES INCLUDING DEFAULTS)';FOR column_, default_ IN SELECT column_name::text, REPLACE(column_default::text, source_schema, dest_schema) FROM information_schema.COLUMNS WHERE table_schema = dest_schema AND TABLE_NAME = object AND column_default LIKE 'nextval(%' || source_schema || '%::regclass)' LOOP EXECUTE 'ALTER TABLE ' || buffer || ' ALTER COLUMN ' || column_ || ' SET DEFAULT ' || default_; END LOOP; END LOOP; END; $BODY$ LANGUAGE plpgsql VOLATILE;";

    spawnConnection(connectionName, function _cloneSchemaFunction(client, _cb) {
      client.query(_cloneSchema, function _createSchema(err, result) {
        return _cb(err);
      });
    }, cb);
  }
